...
inline int DivDown(int a, int b) { return a / b; }
...
#define N 512
void random_fill(size_t n, size_t m, float *arr) {
    for (size_t i = 0; i < n; ++i)
        for (size_t j = 0; j < m; ++j)
            arr[i*N+j] = rand();  // 32bit integer random number
}
float A[N][N], B[N][N], C[N][N];

uint64_t test_gemm()
{
  random_fill(N, N, &A[0][0]);
  random_fill(N, N, &B[0][0]);
  timespec start = timer_start();
  tars::device::x86::avx2_col_major_sgemm(N, N, N, 1.0, &A[0][0], N, &B[0][0], N, 0.0, &C[0][0], N);
  return timer_end(start);
}
#define TIMES 50
double calc_gflops(uint64_t time_ns) {
    return 2.0 * N * N * N * TIMES / time_ns;
}
int main() {
  srand(114514);
  random_fill(N, N, &A[0][0]);
  ...
  printf("%.2f\n", C[1][2]);
  printf("warm up time: %lld ms\n", sum / 1000000);
  // measure
  uint64_t total = test_func(test_gemm, TIMES);
  printf("%.2f\n", C[1][2]);
  printf("total time of block: %lld ms, %.2f GFLOPS\n", total / 1000000, calc_gflops(total));
  return 0;
